...
(function() {
  var CONSUL_URL, ConsulAgent, EventEmitter, HEALTH_POLL_MS, VERBOSE, WATCH_S, _, async, healthyInstances, last_index, log, makeWatchQuery, request, unhealthyInstances, util,
  ...
  module.exports = ConsulAgent = (function(superClass) {
    extend(ConsulAgent, superClass);
  ...
  ConsulAgent.prototype.getServiceHealth = function(service_name, cb) {
    ...
    if (indexOf.call(this.known_services, service_name) >= 0 && (healthy_instances = this.known_instances[service_name])) {
      ...
      return this.watchServiceHealth(service_name, 0, (function(_this) {
        return function(err, service_instances) {
          ...
          if (!(indexOf.call(_this.known_services, service_name) >= 0)) {
  ...
  ConsulAgent.prototype.updateServiceHealth = function(service_name, index, cb) {
    ...
    return this.watchServiceHealth(service_name, index, (function(_this) {
      return function(err, service_instances) {
        var dead_ids, getServiceID, healthy_ids, healthy_instances, known_ids, new_ids, ref;
        getServiceID = function(ins) {
          return ins.Service.ID;
        };
        healthy_instances = healthyInstances(service_instances);
        healthy_ids = healthy_instances.map(getServiceID);
        known_ids = ((ref = _this.known_instances[service_name]) != null ? ref.map(getServiceID) : void 0) || [];
        new_ids = healthy_ids.filter(function(i) {
          return indexOf.call(known_ids, i) < 0;
        });
        dead_ids = known_ids.filter(function(i) {
          return indexOf.call(healthy_ids, i) < 0;
        });
