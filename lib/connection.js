...
(function() {
  var Connection, DEFAULT_CONNECT, DEFAULT_PROTO, EventEmitter, PING_INTERVAL, VERBOSE, _, log, makeAddress, randomString, ref, util, zmq,
  ...
  ref = require('./helpers'), log = ref.log, randomString = ref.randomString, makeAddress = ref.makeAddress;
  ...
  module.exports = Connection = (function(superClass) {
    ...
    function Connection(options) {
      ...
      this.address = makeAddress(this.proto, this.host, this.port);
      this.connect();
    }
    ...
    Connection.prototype.send = function(message, on_response) {
      ...
      message.service || (message.service = this.service_instance.name);
      if (on_response != null) {
    ...
    Connection.prototype.resendSubscribe = function(subscription) {
      var existing_cb, subscribe_msg;
      existing_cb = this.pending_responses[subscription.id];
      delete this.pending_responses[subscription.id];
      subscribe_msg = {
        id: subscription.id,
        kind: 'subscribe',
      ...
      return this.send(subscribe_msg, existing_cb);
    };
    ...
    Connection.prototype.sendPing = function(ping_again) {
      ...
      this.pingTimeout = setTimeout(this.pingDidTimeout.bind(this), PING_INTERVAL);
      return this.last_ping = this.send({
        kind: 'ping'
      }, (function(_this) {
        return function(err, pong) {
          if (pong === 'hello') {
            if (VERBOSE) {
              log.i("[" + _this.service_instance.id + "] New ping response");
            }
            _this.timed_out = false;
            _this.emit('connect');
          } else if (pong !== 'pong') {
            if (VERBOSE) {
              log.e("[" + _this.service_instance.id + "] Ping response invalid");
            }
            _this.emit('failure');
          } else {
            if (VERBOSE > 1) {
    ...
    Connection.prototype.pingDidTimeout = function() {
      log.e("[" + this.service_instance.id + " " + this.service_instance.host + "] Ping timed out");
      this.timed_out = true;
