...
(function() {
  var CONNECTION_KEEPALIVE_MS, CONNECTION_LINGER_MS, CONNECTION_RETRY_MS, Client, Connection, EventEmitter, KEEPALIVE, REGISTRY_HOST, REGISTRY_PORT, VERBOSE, _, emitters, helpers, log, randomString, util,
    slice = [].slice;
    ...
    _trySubscribe = function() {
      return me.getServiceConnection(service_name, function(err, service_connection) {
        ...
        if (service_connection != null) {
          ...
          return subscription.connection.once('reconnect', function() {
  ...
  Client.prototype.resubscribeAll = function() {
    var ref, results, service_id, subscription, subscription_id;
    ref = this.service_subscriptions;
    results = [];
    for (subscription_id in ref) {
      subscription = ref[subscription_id];
  ...
  Client.prototype.closeConnection = function(service_instance) {
    ...
    doClose = function() {
      if (VERBOSE) {
        log.d("[closeConnection] Connection to " + service_name + " closed after " + (CONNECTION_LINGER_MS / 1000) + "s");
      }
