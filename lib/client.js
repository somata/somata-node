...
(function() {
  ...
  Client = (function(superClass) {
    ...
    Client.prototype.deregisteredService = function(err, old_service) {
      ...
      delete this.service_connections[old_service.id];
      if (subscriptions = this.service_subscriptions[old_service.id]) {
        ...
        return function(err, connection) {
          ...
          if (connection != null) {
            ...
            return connection.on('connect', function() {
              ...
              s.cb = cb;
              (base = _this.service_subscriptions)[name = connection.service.id] || (base[name] = []);
              return _this.service_subscriptions[connection.service.id].push(s);
            });
          } else {
    ...
    Client.prototype.resubscribe = function(subscription) {
      return this.getConnection(subscription.service, (function(_this) {
        return function(err, connection) {
          ...
          if (connection != null) {
            return connection.on('connect', function() {
              return connection.subscribe.apply(connection, [subscription.type].concat(slice.call(subscription.args), [subscription.cb]));
            });
          } else {
