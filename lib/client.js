...
(function() {
  var CONNECTION_LINGER_MS, CONNECTION_RETRY_MS, CONNECTION_TIMEOUT_MS, Client, Connection, ConsulAgent, EventEmitter, KEEPALIVE, PREFIX, VERBOSE, helpers, log, util, _,
    __slice = [].slice;
  ...
  Client.prototype.subscribe = function() {
    var args, cb, service_name, type, _i, _retrySubscribe;
    ...
    _retrySubscribe = ((function(_this) {
      return function() {
    ...
    return this.getServiceConnection(service_name, (function(_this) {
      return function(err, service_connection) {
        ...
        if (err) {
          ...
          return setTimeout(_retrySubscribe, CONNECTION_RETRY_MS);
        } else {
          ...
          return _this.consul_agent.once('deregister:services/' + service.ID, function() {
            if (service_connection.pending_responses[subscription.id] != null) {
  ...
  Client.prototype.unsubscribe = function(_sub) {
    return _.chain(this.service_subscriptions).pairs().where(_sub).map((function(_this) {
      return function(_arg, _cb) {
        var sub, sub_id;
  ...
  Client.prototype.getServiceConnection = function(service_name, cb) {
    ...
    if (service_connection = this.service_connections[service_name]) {
      if (service_connection.connected) {
        ...
        return this.connection_manager.once('connected:' + service_name, function(service_connection) {
          return cb(null, service_connection);
        });
      }
    } else {
      this.service_connections[service_name] = {
        connected: false
      };
      return this.consul_agent.checkServiceHealth(service_name, 0, (function(_this) {
  ...
  Client.prototype.saveServiceConnection = function(service, service_connection) {
    ...
    this.consul_agent.known_services.push(service.Service);
    if (KEEPALIVE) {
