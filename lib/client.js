...
Client = (function(_super) {
  ...
  Client.prototype.remote = function() {
    ...
    return this.getConnection(service, (function(_this) {
      return function(err, connection) {
        if (connection != null) {
          return connection.method.apply(connection, [method].concat(__slice.call(args), [cb]));
        } else {
  ...
  Client.prototype.subscribe = function() {
    var args, cb, id, service, service_name, subscription, type, _i;
    service = arguments[0], type = arguments[1], args = 4 <= arguments.length ? __slice.call(arguments, 2, _i = arguments.length - 1) : (_i = 2, []), cb = arguments[_i++];
        ...
        if (connection != null) {
          ...
          return connection.on('timeout', function() {
            ...
            return setTimeout(function() {
              return _this.subscribe.apply(_this, [service, type].concat(__slice.call(args), [cb]));
            }, 500);
          });
        } else {
  ...
  Client.prototype.sendSubscription = function(connection, subscription, cb) {
    ...
    subscription.cb = cb;
    (_base = this.service_subscriptions)[_name = subscription.service.id] || (_base[_name] = []);
  ...
  Client.prototype.closeConnections = function() {
    var connection, service_name, _ref;
    _ref = this.service_connections;
    for (service_name in _ref) {
      connection = _ref[service_name];
      connection.close();
    }
