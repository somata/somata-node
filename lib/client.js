...
(function() {
  var CONNECTION_KEEPALIVE_MS, CONNECTION_LINGER_MS, CONNECTION_RETRY_MS, Client, Connection, ConsulAgent, EventEmitter, KEEPALIVE, PREFIX, VERBOSE, _, emitters, helpers, log, randomString, util,
    slice = [].slice;
  ...
  Client.prototype.call = function() {
    var args, cb, i, method, service_name;
    service_name = arguments[0], method = arguments[1], args = 4 <= arguments.length ? slice.call(arguments, 2, i = arguments.length - 1) : (i = 2, []), cb = arguments[i++];
    return this.remote.apply(this, [service_name, method].concat(slice.call(args), [cb]));
  };

  Client.prototype.remote = function() {
    var args, cb, i, message_id, method, service_name;
    service_name = arguments[0], method = arguments[1], args = 4 <= arguments.length ? slice.call(arguments, 2, i = arguments.length - 1) : (i = 2, []), cb = arguments[i++];
  ...
  Client.prototype.subscribe = function() {
    var _trySubscribe, args, cb, i, me, service_name, subscription_id, type;
    service_name = arguments[0], type = arguments[1], args = 4 <= arguments.length ? slice.call(arguments, 2, i = arguments.length - 1) : (i = 2, []), cb = arguments[i++];
    if (typeof cb !== 'function') {
    ...
    _trySubscribe = function() {
      return me.getServiceConnection(service_name, function(err, service_connection) {
        ...
        if (service_connection != null) {
          ...
          return me.consul_agent.once('deregister:services/' + service.ID, function() {
  ...
  Client.prototype.on = function() {
    var args, cb, i, service_name, type;
    service_name = arguments[0], type = arguments[1], args = 4 <= arguments.length ? slice.call(arguments, 2, i = arguments.length - 1) : (i = 2, []), cb = arguments[i++];
    return this.subscribe.apply(this, [service_name, type].concat(slice.call(args), [cb]));
  };
  ...
  Client.prototype.connectToService = function(instance) {
    var connection, ref;
    if (connection = (ref = this.service_connections[instance.Service.Service]) != null ? ref[instance.Service.ID] : void 0) {
  ...
  Client.prototype.saveServiceConnection = function(service, service_connection) {
    ...
    (base = this.service_connections)[name = service.Service] || (base[name] = {});
  ...
  Client.prototype.killConnection = function(instance) {
    var connection, doClose, ref, service_id, service_name;
    service_name = instance.Service.Service;
    service_id = instance.Service.ID;
    if (VERBOSE) {
      log.w('[killConnection] ' + service_name);
    }
    if (connection = (ref = this.service_connections[service_name]) != null ? ref[service_id] : void 0) {
